(function(){
  window.onload = function () {
    ...
    if (
      document.querySelectorAll(".technologie").length > 0 &&
      document.querySelectorAll(".progress-procent[data-progress]").length > 0
    ) {
      // Get all elements with 'data-progress' attribute and run the 'AnimateProgress' funcion with each one
      document
        .querySelectorAll(".progress-procent[data-progress]")
